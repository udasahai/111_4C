...
void socket_handler()
{
		...
		SSL_set_fd(sslClient, filedes);
		SSL_connect(sslClient);			
	
		char id[256]; 
		sprintf(id,"ID=%s\n",myID);	
			SSL_write(sslClient, id, strlen(id));
}
...
void processInput()
{
	...
	int size ; 
		...
		size = SSL_read(sslClient,&Buffer,256);
...
while( (opt = getopt_long(argc, argv, "l:p:s:i:h:", long_opts, NULL)) != -1)
  {
	  switch(opt)
	  {
		 case 'i':
...
void loop()
{
	...
	if(!exit_flag)
	{
		...
		sprintf(dataVal, "%s %0.1f\n", timeString, temp);
			...
			SSL_write(sslClient,dataVal,strlen(dataVal));
